...
class get_table():
    ...
    def golden(name, id, pwd, date):
        ...
        table.to_csv("results/{}_{}.csv".format(name, date), encoding='shift-jis')
        time.sleep(1)
        #logout
    ...
    def get_idpwd(n):
        ...
        for i in range(n):
            book = xl.load_workbook("idpass.xlsx",data_only=True)
            sheet = book["nip"]
...
niplist = get_table.get_idpwd(125) #get_idpwdの引数は読み込む法人数
